...
# A method taken as this.method has a hidden first `this` parameter; Bind
# without passing `this` first shifts every argument (use ObjBindMethod).
RE_AHK_METHOD_BIND_WITHOUT_THIS = re.compile(r"\bthis\.[A-Za-z_]\w*\.Bind\((?!\s*this\b)")
...
def scan_ahk_risks(file_index: dict[str, dict[str, object]]) -> list[dict[str, object]]:
    ...
    for repo_path, meta in sorted(file_index.items()):
        for line_number, line in enumerate(str(meta["text"]).splitlines(), start=1):
            ...
            if RE_AHK_METHOD_BIND_WITHOUT_THIS.search(code):
                issues.append({"type": "ahk_method_bind_without_this", "file": repo_path, "line": line_number, "message": "this.method.Bind(...) binds the first argument as `this`; use ObjBindMethod(this, \"method\", ...)."})
...
def validate_workspace_contract(repo_root: Path) -> list[dict[str, object]]:
    return [{"type": "workspace_contract", "file": GOVERNANCE_FILE, "message": problem} for problem in shared_contract.problems(repo_root)]
