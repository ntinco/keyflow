...
2. Windows (no machine available yet): start keyflow, run `platforms/windows/tools/selftest.ahk` and paste its report (covers autocorrect, immediate snippets, `;;` and its clipboard restore, Win+Esc on every monitor and on a maximized window). Then, manually: confirm profiles load without JSON errors, autocorrect/snippets/SAP transactions work, `=ED_OPTIONS` and `=<ymt code>` are sent without `/n`, SAP hotstrings wait for the ending character, F12 pastes `.txt` but ignores `.exe`, and Alt+P plays a media path that contains spaces. Also: Alt+4…0 with the cursor in a SAP data field leaves that field intact (Ctrl+/ focuses the command field first).
